...
public class lab5 {
    ...
    public static void main(String[] args){
		...
		for (int i = 0; i < arrayList.size(); i++) {	//gets opcode for each instruction
		    //System.out.println(arrayList.get(i).getInstruction());
        ...
        int newGHR = 2;
        if (args.length == 1) {
            //System.out.println(program.getGHR());
            program.initializeBranchPredictor();
            while (running) {
            ...
            if (args[1].length() > 1) {
                ...
                // NO SCRIPT, GHR INPUT
                try{
                    newGHR= Integer.parseInt(args[1]);
                }
                catch (NumberFormatException e) {
                    System.err.println("Invalid GHR.\nUsage: java lab5 file.asm [script file] [int GHR]");
                    running = false;
                }
                program.setGHR(newGHR);
                program.initializeBranchPredictor();
                //System.out.println(program.getGHR());
                while (running) {
                    System.out.print("mips> ");
            ...
            try{
                newGHR= Integer.parseInt(args[2]);
            }
            catch (NumberFormatException e) {
                System.err.println("Invalid GHR.\nUsage: java lab5 file.asm [script file] [int GHR]");
                running = false;
